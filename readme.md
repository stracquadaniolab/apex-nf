...
- **Software:** 
    - **Nextflow:** A workflow management system. Install it from [Nextflow's website](https://www.nextflow.io/).
    - Access to a Command Line Interface (CLI).

After installing the `stracquadniolab/opentrons-protein-screening-pipeline`, a Docker image containing all necessary packages, including Opentrons App, will be downloaded. Docker containers provide a consistent environment for the software to run, ensuring that it works the same way on any system. This allows for the pipeline to run the workflows effortlessly.

## Installation
To set up the `opentrons-protein-screening-pipeline`, open your Terminal or Command Prompt and execute:

```
nextflow pull stracquadniolab/opentrons-opentrons-protein-screening-pipeline

```

This command downloads and sets up the `stracquadniolab/pipeline` on your system.
