...
def get_parameters(json_file: str) -> dict:
    """
    This function parses the json file with the parameters for
    the experiment and returns them as a dictionary.
    """
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    pipette_tipracks = [
        protocol.load_labware(load_name=params["pipette_tiprack_name"], location=i)
        for i in params["pipette_tiprack_slots"]
    ]
    ...
    ######### SPOTTING ##########
    for plate, source, destination, volume, weight in zip(
        agar_plates,
        csv_params.source_wells,
        csv_params.destination_wells,
        csv_params.spotting_volume,
        csv_params.agar_plate_weight,
    ):
        pipette.well_bottom_clearance.dispense = 1
        pipette.pick_up_tip()
        pipette.mix(
            repetitions=3, volume=5, location=transformation_plate[source], rate=2
        )
        pipette.aspirate(
            volume=volume + params["dead_volume"],
