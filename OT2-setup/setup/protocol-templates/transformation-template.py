...
def read_csv_file(csv_file):
    ...
    csv_parameters = collections.namedtuple("csv_parameters", ["cells_source_wells", "cells_volume", "dna_source_wells", "dna_volume", "soc_source_wells", "soc_volume", "transformation_well"])

    return csv_parameters(cells_source_wells, cells_volume, dna_source_wells, dna_volume, soc_source_wells, soc_volume, transformation_well)
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    source_plate = protocol.load_labware(params["dna_plate_name"], params["dna_plate_slot"]) # plate containing dna to be transformed
    ...
    pipette_1_tipracks = [protocol.load_labware(load_name=params["pipette_1_tiprack_name"], location=i) for i in params["pipette_1_tiprack_slots"]]
    pipette_1 = protocol.load_instrument(instrument_name=params["pipette_1_name"], mount=params["pipette_1_mount"], tip_racks=pipette_1_tipracks) # p20 pipette

    pipette_2_tipracks = [protocol.load_labware(load_name=params["pipette_2_tiprack_name"], location=i) for i in params["pipette_2_tiprack_slots"]]
    pipette_2 = protocol.load_instrument(instrument_name=params["pipette_2_name"], mount=params["pipette_2_mount"], tip_racks=pipette_2_tipracks) # p300 pipette
    ...
    pipette_2.distribute(volume=csv_params.cells_volume, # volume of competent cells to be transferred
                    source=[source_plate.wells_by_name()[well] for well in csv_params.cells_source_wells], # transfer from the specified well in the resevoir containing competent cells
                    dest=[transformation_plate.wells_by_name()[well] for well in csv_params.transformation_well], # transfer to transformation plate in the thermocycler
                    mix_before=(1,50)) # resuspend the competent cells once before transfering into the transformation plate with the total volume to be trasfered, volume not specified sets the default to the max volume of pipette

    # step 3: Transfer DNA into wells of the transformation plate.
    pipette_1.well_bottom_clearance.aspirate = 0.5 # aspiration height of the pipette (when volume of DNA is low)
    
    for i in zip(csv_params.dna_source_wells, csv_params.transformation_well, csv_params.dna_volume, csv_params.cells_volume):
        ...
        pipette_1.dispense(volume=i[2], location=transformation_plate.wells_by_name()[i[1]])
        mixing_volume = (i[2]+i[3])/2
        pipette_1.aspirate(volume=mixing_volume, location=transformation_plate.wells_by_name()[i[1]])
        pipette_1.dispense(volume=mixing_volume, location=transformation_plate.wells_by_name()[i[1]])
        pipette_1.aspirate(volume=mixing_volume, location=transformation_plate.wells_by_name()[i[1]])
        pipette_1.dispense(volume=mixing_volume, location=transformation_plate.wells_by_name()[i[1]])
    ...
    thermocycler_mod.set_block_temperature(temperature=params["heat_temp"], 
                                           hold_time_seconds=params["heat_time"])
    ...
    for i in zip(csv_params.soc_source_wells, csv_params.transformation_well, csv_params.soc_volume, csv_params.dna_volume, csv_params.cells_volume):
        pipette_2.pick_up_tip()
        pipette_2.aspirate(volume=i[2], location=source_plate.wells_by_name()[i[0]])
        pipette_2.dispense(volume=i[2], location=transformation_plate.wells_by_name()[i[1]])
        mixing_volume = (i[2]+i[3]+i[4])/2
        for _ in range(3):
            pipette_2.aspirate(volume=mixing_volume, location=transformation_plate.wells_by_name()[i[1]])
            pipette_2.dispense(volume=mixing_volume, location=transformation_plate.wells_by_name()[i[1]])
